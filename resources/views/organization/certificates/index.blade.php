...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-2">Manage and track certificates issued by your organization</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $certificates->where('created_at', '>=', now()->startOfMonth())->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $certificates->where('is_verified', true)->count() }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        @foreach($certificates as $certificate)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-sm text-gray-500">{{ $certificate->event_date->format('M j, Y') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                                        @if($certificate->type === 'volunteer') bg-green-100 text-green-800
                                        @elseif($certificate->type === 'completion') bg-blue-100 text-blue-800
                                        @else bg-purple-100 text-purple-800 @endif">
                                        {{ ucfirst($certificate->type) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $certificate->hours_completed }} hrs
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $certificate->issued_date->format('M j, Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        @if($certificate->file_path)
                                            <a href="{{ route('organization.certificates.download', $certificate) }}" 
                                               class="text-blue-600 hover:text-blue-900" title="Download PDF">
                                        ...
                                        <a href="{{ route('organization.certificates.show', $certificate) }}" 
        ...
        @else
            <div class="px-6 py-12 text-center">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">No certificates issued</h3>
                <p class="mt-1 text-sm text-gray-500">Get started by generating certificates for completed events.</p>
            </div>
        @endif
    </div>
</div>
